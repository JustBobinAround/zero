...
// impl<T> Extract<T, RequestBody, RequestBody> for Body<HashMap<String, String>> {
//     fn from_request(instance: PhantomData<T>, body: RequestBody) -> Result<Self, ()> {
//         let mut parser = StrParser::from_str(body.as_str());
//         let query = RequestQuery::parse(&mut parser).map_err(|_| ())?;
//         Ok(Body(query.parameters))
//     }
...
// impl<T> Extract<T, Request>
macro_rules! impl_extract {
    (|$part_name2:ident: $part_ty2:tt <$part_subty2:ident>| {
        $($part_name:ident: $part_ty:tt $(<$part_subty:ident$(:$trait:ident)?>)?),*
    }) => {
        impl<$part_subty2 $($(,$part_subty$(:$trait)?)?)*> Extract<$part_subty2, InstanceRequest<$part_subty2>, Self> for ($part_ty2<$part_subty2>, $($part_ty$(<$part_subty>)?,)*) {
            fn from_request(_instance: PhantomData<$part_subty2>, req: InstanceRequest<$part_subty2>) -> Result<Self, ()> {
                Ok((req.$part_name2, $(<$part_ty$(<$part_subty>)?>::from_request(_instance, req.$part_name)?,)*))
            }
        }
    };
    (|$instance_ty: ident| {
...
impl_extract!(|instance: Instance<T>| {});
impl_extract!(|T|{
    method: Method
});
impl_extract!(|instance: Instance<T>|{
